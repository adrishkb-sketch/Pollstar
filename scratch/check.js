...
function computeSemanticSimilarity(userAns, correctAns) {
  ...
  const parseClausesBasic = (text) => {
    const separators = /\b(?:and|but|while|though|although|whereas|if|unless|because|since|so|yet|or|nor|as well as)\b|[,;\.\-\(\)]/gi;
  ...
  const wordExistsInClause = (word, clause) => {
  ...
  const areInSameBasicClause = (clauses, w1, w2) => {
...
const testCases = [
  {
    name: "User Swap Case: plants release CO2 and absorb O2",
    correct: "plants release oxygen and absorb carbon dioxide",
    user: "plants release carbon dioxide and absorb oxygen",
    expectedMin: 0.0,
    expectedMax: 0.1
  },
  {
    name: "User Correct Phrase: plants absorb carbon dioxide and release oxygen",
    correct: "plants release oxygen and absorb carbon dioxide",
    user: "plants absorb carbon dioxide and release oxygen",
    expectedMin: 0.85,
    expectedMax: 1.0
  },
  {
